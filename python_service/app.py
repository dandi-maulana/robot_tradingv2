...
DB_CONFIG = {
    ...
    'database': 'robot_trading' # Samakan dengan .env Laravel
}
                    ...
                    if state['tg_active']:
                        ...
                        if state["tg_last_candle"] != candle_id:
                            ...
                            if tg_phase == "IDLE" and (mm % 5 == 2):
                                ...
                                hasil_teks = "PROFIT" if is_win else "LOSS"
...
def status_all():
    conn = get_db_connection()
    if not conn: return jsonify({"active_markets": [], "balance": global_demo_balance})
    c = conn.cursor()
    c.execute("SELECT market FROM market_states WHERE is_running = 1")
    active_markets = [row[0] for row in c.fetchall()]
    conn.close()
    return jsonify({"active_markets": active_markets, "balance": global_demo_balance})

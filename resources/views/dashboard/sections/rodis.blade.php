<div id="view-rodis" class="fade-in hidden">
    <div
        class="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-3xl p-8 shadow-lg border border-indigo-500/30 mb-8 relative overflow-hidden text-white">
        <h2 class="text-3xl font-extrabold mb-2 text-white">Robot <span class="text-indigo-400">RODIS</span>
            (Auto-Trade)</h2>
        <p class="text-indigo-200 mb-8 max-w-2xl">Atur target Signal Loss Anda. Robot akan otomatis bersiaga,
            membaca candle, dan <b>mengeksekusi order Buy/Sell langsung ke broker</b> tanpa campur tangan Anda.</p>

        <div class="bg-slate-800/50 backdrop-blur border border-indigo-500/20 p-6 rounded-2xl mb-8 relative z-10">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    <label class="block text-xs font-bold text-indigo-300 uppercase mb-2">Pilih Market
                        Aktif</label>
                    <select id="rodis-market-select"
                        class="w-full px-4 py-3 bg-slate-900 border border-indigo-500/50 rounded-xl font-bold text-white outline-none focus:border-indigo-400"></select>
                </div>
                <div>
                    <label class="block text-xs font-bold text-indigo-300 uppercase mb-2">Target Signal
                        Loss</label>
                ...
                <div>
                    ...
                    <select id="rodis-duration"
                        class="w-full px-4 py-3 bg-slate-900 border border-indigo-500/50 rounded-xl font-bold text-white outline-none">
            ...
            <div class="mt-6 flex justify-between items-center border-t border-indigo-500/20 pt-6">
                ...
                <button id="btn-rodis-toggle" onclick="toggleRodisBot()"
                    class="px-8 py-4 bg-indigo-500 hover:bg-indigo-400 text-white font-extrabold rounded-xl shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all text-lg tracking-wide">
                    ▶ NYALAKAN RODIS
                </button>
            </div>
        </div>
        ...
        <div id="rodis-terminal" class="shadow-inner"
            ...
            <div class="text-indigo-400 opacity-50 mb-2">Sistem siap. Hubungkan Market di Monitor, atur form di
                atas, lalu klik Nyalakan RODIS.</div>
        </div>
    </div>
</div>

...
function renderLocalChart(historyData, marketName) {
    const chartContainer = document.getElementById('local-chart-container');
...
function showView(viewName) {
    ...
    // 🔥 hide semua view (AMAN)
    safeHide('view-dashboard');
    safeHide('view-detail');
    safeHide('view-trade');
    safeHide('view-history');
    safeHide('view-rodis');
    safeHide('view-doji');

    // 🔥 show target view
    safeShow('view-' + viewName);
        ...
        fetch(`${API_BASE}/status_all`)
            ...
            .then(data => {
                if (data.balance !== undefined && data.balance !== null) {
                    ...
                    if (el) el.innerText = formatCurrency(data.balance);
                }
...
function syncPlayStopButton() {
    ...
    // Lock tombol saat start/stop sedang diproses agar tidak perlu spam klik
    btn.disabled = isToggleBusy;

    const isActive = activeMarketsList.length > 0 || isRunning;
...
function showConfirm(message, onYes) {
    ...
    overlay.className =
        "fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-[9999] animate-fadeIn";
...
function startAllMarkets(event) {
    if (isToggleBusy) return;
    const token = document.getElementById('token').value;
    const accountId = document.getElementById('account-id').value;
    if (!token || !accountId) {
        showPopup("⚠️ Harap isi Access Token & Target Account ID terlebih dahulu!", "error");
        return;
    }

    // Optimistic intent: biar tombol langsung berubah tanpa perlu spam klik
    isRunning = true;
    isToggleBusy = true;
    toggleRequestedAt = Date.now();
    toggleTimeoutShown = false;
    syncPlayStopButton();

    const btn = event && event.currentTarget ? event.currentTarget : event.target;
...
let isRunning = false;
let isToggleBusy = false;
let toggleRequestedAt = 0;
let toggleTimeoutShown = false;
const TOGGLE_SYNC_TIMEOUT_MS = 15000;

function toggleMarkets(event) {
    if (isToggleBusy) return;
    const isActive = activeMarketsList.length > 0 || isRunning;
    if (isActive) {
        stopAllMarkets(event);
    } else {
        startAllMarkets(event);
    }
}
    ...
    fetch(`${API_BASE}/toggle_telegram_all`, {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                // 🔥 FIX: pakai response server, bukan toggle manual
                if (data.active !== undefined) {
                    isMassTelegramActive = data.active;
                }

                if (isMassTelegramActive) {
                    btn.innerHTML = '🔕 Stop Sinyal';
                    btn.classList.remove('bg-blue-600', 'hover:bg-blue-700');
                    btn.classList.add('bg-red-600', 'hover:bg-red-700');
                } else {
                    btn.innerHTML = '📲 Sinyal Massal';
                    btn.classList.remove('bg-red-600', 'hover:bg-red-700');
...
function refreshDashboardStatus() {
    fetch(`${API_BASE}/status_all`)
        ...
        .then(data => {
            ...
            activeMarketsList = data.active_markets || [];
            const serverActive = activeMarketsList.length > 0;

            // Saat proses start/stop, jangan biarkan polling membalik tombol sebelum server sinkron.
            if (isToggleBusy) {
                if (serverActive === isRunning) {
                    isToggleBusy = false;
                } else if (!toggleTimeoutShown && (Date.now() - toggleRequestedAt) > TOGGLE_SYNC_TIMEOUT_MS) {
                    toggleTimeoutShown = true;
                    isToggleBusy = false;
                    isRunning = serverActive;
                    showPopup("Status bot belum sinkron. Coba tunggu / refresh halaman.", "error");
                }
            } else {
                // Idle: sync intent dengan status server biar toggle akurat.
                isRunning = serverActive;
            }
            syncPlayStopButton();

            document.querySelectorAll('.market-card').forEach(card => {
                if (activeMarketsList.includes(card.getAttribute('data-market')))
            ...
            if (streakContainer && streakList) {
                ...
                if (!data.market_streaks || Object.keys(data.market_streaks).length === 0) {
                    streakList.innerHTML =
                        `<span class="text-xs text-gray-400 italic">Semua market berhenti</span>`;
                    return;
                }
            ...
            if (btn) {
                isMassTelegramActive = (data.tg_active_count || 0) > 0;
            ...
            // DOJI ANALYTICS
            if (typeof renderDojiAnalytics === "function") {
...
function openMarketDetail(marketName) {
    ...
    console.log("OPEN DETAIL:", marketName);

    // 🔥 amanin element
    const nameEl = document.getElementById('detail-market-name');
    if (nameEl) nameEl.innerText = marketName;

    // 🔥 pindah view DULU (biar DOM siap)
    try {
        showView('detail');
    } catch (e) {
        console.warn("showView error:", e);
    }
    ...
    if (localChartInstance) {
        ...
        const chartEl = document.getElementById('local-chart-container');
        if (chartEl) {
            chartEl.innerHTML =
...
function refreshDetailData() {
    ...
    fetch(`${API_BASE}/data?market=${encodeURIComponent(currentMarket)}`)
        .then(res => res.json())
        .then(data => {
            ...
            if (valTotal) valTotal.innerText = data.stats?.total_trade ?? 0;

            let sigLoss = calculateSigLoss(data.history);
            const valSigLoss = document.getElementById('val-sig-loss');
            if (valSigLoss) valSigLoss.innerText = sigLoss;
            ...
            const valSigWin = document.getElementById('val-sig-win');
            if (valSigWin) valSigWin.innerText = sigWin;

            const valHijau = document.getElementById('val-hijau');
            if (valHijau) valHijau.innerText = data.stats?.total_hijau ?? 0;

            const valMerah = document.getElementById('val-merah');
            if (valMerah) valMerah.innerText = data.stats?.total_merah ?? 0;
            ...
            if (data.telegram) {
                ...
                const valTarget = document.getElementById('val-target-op');
                ...
                if (data.telegram.active) {
                    ...
                    if (sigLoss > 0 && valTarget) {
                        valTarget.innerText = data.telegram.trade_counter + 1;
                    }
                } else {
                    if (btn) {
                    ...
                    if (inLoss) inLoss.disabled = false;
                    if (valTarget) valTarget.innerText = 0;
                }
            }
            ...
            const pagination = document.getElementById('detail-pagination-controls');
...
function renderDojiAnalytics(dojiData) {
    ...
    dojiData.forEach(item => {
        const mktObj = allMarkets.find(x => x.id === item.market) || {
            name: item.market,
            icon: '📈'
        };
